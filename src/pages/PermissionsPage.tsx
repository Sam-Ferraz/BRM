...
import { api, type PermissionsMatrix, type ManagedUserRole } from "@/lib/api-client"
import { useAuth } from "@/hooks/use-auth"
import { useToast } from "@/hooks/use-toast"

const ROLE_ORDER: ManagedUserRole[] = ["admin", "manager", "broker", "sdr", "administrative"]
...
export default function PermissionsPage() {
  ...
  const fetchMatrix = useCallback(async () => {
    try {
      setLoading(true)
      const res = await api.userMgmt.getPermissionsMatrix()
      setMatrix(res.data)
      setPending(new Map())
    } catch (err) {
      toast({
        title: "Erro",
        description: err instanceof Error ? err.message : "Falha ao carregar matriz",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const getOriginal = (role: ManagedUserRole, permissionId: number): boolean => {
  ...
  const toggle = (role: ManagedUserRole, permissionId: number, currentAllowed: boolean) => {
    const key = `${role}:${permissionId}`
    const newAllowed = !currentAllowed
    setPending((prev) => {
      const next = new Map(prev)
      const original = getOriginal(role, permissionId)
  ...
  const isChecked = (role: ManagedUserRole, permissionId: number, defaultAllowed: boolean): boolean => {
    const key = `${role}:${permissionId}`
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.userMgmt.savePermissionsMatrix(updates)
      toast({
                ...
                <table className="w-full border-collapse">
                  <thead className="sticky top-0 bg-card z-10">
                    <tr className="border-b">
                      <th className="text-left py-2 pr-4 text-sm font-semibold">Rotina</th>
                      {ROLE_ORDER.map((r) => (
                        <th
                          key={r}
...
function ModuleRows({
  ...
  isChecked: (role: ManagedUserRole, permissionId: number, defaultAllowed: boolean) => boolean
  onToggle: (role: ManagedUserRole, permissionId: number, currentAllowed: boolean) => void
}) {
  return (
    <>
