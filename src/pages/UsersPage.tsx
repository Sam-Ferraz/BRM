...
const ROLE_INFO: Record<UserRole, { label: string; description: string; className: string }> = {
  admin: {
    label: "Admin",
                ...
                <TableBody>
                  {users.map((user) => (
                    <TableRow key={user.id} className={user.active ? "" : "opacity-60"}>
                      ...
                      <TableCell>
                        <Badge className={ROLE_INFO[user.role]?.className || ""}>
                ...
                <SelectContent>
                  {(Object.keys(ROLE_INFO) as UserRole[]).map((r) => (
                    <SelectItem key={r} value={r}>
                      <div>
                        <div className="font-medium">{ROLE_INFO[r].label}</div>
