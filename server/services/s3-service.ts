...
class S3Service {
   ...
   * Upload file to S3
   */
  async uploadFile(fileBuffer: Buffer, originalFilename: string, contentType: string, productId: number): Promise<string> {
    try {
   ...
   * Get file from S3
   */
  async getFile(filePath: string): Promise<S3FileData> {
    try {
