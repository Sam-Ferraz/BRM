...
export class LeadPipelineRepository extends BaseRepository {
  ...
  async findAll(): Promise<LeadPipeline[]> {
    const client = await this.getClient()
    try {
      const result = await client.query(
  ...
  async findById(id: number): Promise<LeadPipeline | null> {
    const client = await this.getClient()
    try {
      const result = await client.query(
        `SELECT p.*, u.name AS manager_name
    ...
    try {
      ...
      const result = await client.query(
        `INSERT INTO lead_pipelines
           (name, hour_start, hour_end, timeout_seconds, manager_user_id, is_active)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`,
        [
          payload.name,
  ...
  async delete(id: number): Promise<boolean> {
    ...
    try {
      ...
      const result = await client.query(
        'DELETE FROM lead_pipelines WHERE id = $1 RETURNING id',
        [id],
      )
