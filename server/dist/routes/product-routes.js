...
export function createProductRoutes(productService) {
    const router = Router();
    ...
    router.post('/:id/images', authenticateToken, uploadSingle, handleUploadErrors, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            ...
            const filePath = await s3Service.uploadFile(req.file.buffer, req.file.originalname, req.file.mimetype, id);
    ...
    router.post('/:id/images/bulk', authenticateToken, uploadMultiple, handleUploadErrors, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            ...
            const uploadPromises = req.files.map(async (file) => {
                const filePath = await s3Service.uploadFile(file.buffer, file.originalname, file.mimetype, id);
    ...
    router.put('/:id/images/:imageId', authenticateToken, async (req, res) => {
        try {
            const id = parseInt(req.params.id);
            const imageId = parseInt(req.params.imageId);
